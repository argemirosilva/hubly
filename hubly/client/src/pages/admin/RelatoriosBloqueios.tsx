...
import { Lock, CheckCircle, XCircle, Calendar, ShieldAlert } from "lucide-react";
import { usePermissoes } from "@/hooks/usePermissoes";

function RelatoriosBloqueiosContent() {
  ...
  const metricas = useMemo(() => {
    ...
    return {
      total, aprovados, recusados, pendentes,
  ...
  const bloqueiosPorProfissional = useMemo(() => {
    ...
    bloqueiosMes.forEach(b => { map[b.profissionalId] = (map[b.profissionalId] ?? 0) + 1; });
    return Object.entries(map).map(([profId, count]) => ({
      nome: profMap[parseInt(profId)] ?? "Profissional", bloqueios: count,
    })).sort((a, b) => b.bloqueios - a.bloqueios);
  }, [bloqueiosMes, profMap]);

  const motivosMaisComuns = useMemo(() => {
    const map: Record<string, number> = {};
    bloqueiosMes.forEach(b => { if (b.motivo) map[b.motivo] = (map[b.motivo] ?? 0) + 1; });
    return Object.entries(map).map(([motivo, count]) => ({ motivo, count })).sort((a, b) => b.count - a.count).slice(0, 5);
  }, [bloqueiosMes]);

  const motivosRecusa = useMemo(() => {
    const map: Record<string, number> = {};
    bloqueiosMes.filter(b => b.status === "recusado").forEach(b => { if (b.motivoRecusa) map[b.motivoRecusa] = (map[b.motivoRecusa] ?? 0) + 1; });
    return Object.entries(map).map(([motivo, count]) => ({ motivo, count })).sort((a, b) => b.count - a.count).slice(0, 5);
  }, [bloqueiosMes]);
  ...
  return (
    <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-6 animate-in-up">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Total de Bloqueios</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">{metricas.total}</div><p className="text-xs text-muted-foreground mt-1">neste mês</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-1"><CheckCircle className="w-4 h-4" /> Aprovados</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-green-600">{metricas.aprovados}</div><p className="text-xs text-muted-foreground mt-1">{metricas.taxaAprovacao}% de aprovação</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-1"><XCircle className="w-4 h-4" /> Recusados</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-red-600">{metricas.recusados}</div><p className="text-xs text-muted-foreground mt-1">taxa de rejeição</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-1"><Calendar className="w-4 h-4" /> Pendentes</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-yellow-600">{metricas.pendentes}</div><p className="text-xs text-muted-foreground mt-1">aguardando aprovação</p></CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie data={distribuicaoStatus} cx="50%" cy="50%" labelLine={false} label={({ name, value }) => `${name}: ${value}`} outerRadius={80} fill="#8884d8" dataKey="value">
                    {distribuicaoStatus.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.fill} />))}
                  </Pie>
                  <Tooltip />
                </PieChart>
              </ResponsiveContainer>
        ...
        {bloqueiosPorProfissional.length > 0 && (
          <Card>
            <CardHeader><CardTitle className="text-base">Bloqueios por Profissional</CardTitle></CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={250}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {motivosMaisComuns.length > 0 && (
          <Card>
            <CardHeader><CardTitle className="text-base">Motivos Mais Comuns</CardTitle><CardDescription>Top 5 motivos de bloqueio</CardDescription></CardHeader>
            <CardContent>
        ...
        {motivosRecusa.length > 0 && (
          <Card>
            <CardHeader><CardTitle className="text-base">Motivos de Rejeição</CardTitle><CardDescription>Top 5 motivos de recusa</CardDescription></CardHeader>
            <CardContent>
              <div className="space-y-3">
...
/** Wrapper com guard de permissão (hooks não podem ficar após return condicional) */
export default function RelatoriosBloqueios() {
  ...
  if (!pode("agendamentosVer")) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[40vh] gap-3">
        <ShieldAlert className="w-10 h-10 text-muted-foreground" />
        <p className="text-sm text-muted-foreground">Você não tem permissão para acessar o relatório de bloqueios.</p>
      </div>
    );
  }

  return <RelatoriosBloqueiosContent />;
}
