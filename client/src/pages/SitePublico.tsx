...
const HERO_IMAGE = "/manus-storage/hubly-public-hero_02d83375.jpg";
...
function ProductWindow() {
  return (
    <figure className="hubly-motion-product-window relative mx-auto w-full max-w-[570px] overflow-hidden rounded-[28px] border border-white/80 bg-[#fffdf9] p-2 shadow-[0_28px_70px_rgba(59,33,21,0.18)]">
      <img
        src="/manus-storage/hubly-agenda-finance-dashboard_7bf3be24.jpg"
...
function HowItWorksPage() {
  const layers = [
    { number: "01", title: "Agenda e clientes no mesmo fluxo", text: "Você organiza horários, bloqueios, pré-agendamentos e pacotes. Cada atendimento já fica ligado ao histórico da cliente, aos retornos e ao Pipeline." },
    { number: "02", title: "Financeiro com leitura inteligente", text: "Sinais, pagamentos parciais, valores em aberto e comissões ficam visíveis. A IA Financeira calcula o score do negócio, mostra alertas, explica os pontos de atenção e responde perguntas sobre a rotina financeira." },
    { number: "03", title: "Automações que respeitam o atendimento", text: "Confirmações, lembretes e mensagens de retorno acompanham o status real do agendamento. Você sabe o que foi enviado e evita repetir a mesma tarefa todos os dias." },
    { number: "04", title: "Marketing que sai da ideia e chega à postagem", text: "Guarde ideias, escreva roteiros, organize o calendário e programe o mesmo conteúdo em canais e horários diferentes. Depois, registre métricas para entender o que gera mais resultado." },
  ];

  return <><section className="relative overflow-hidden bg-[#fffaf4] py-20 sm:py-28"><div className="absolute right-[-6rem] top-[-8rem] h-80 w-80 rounded-full bg-[#f6d777]/30 blur-3xl" /><div className="relative mx-auto max-w-4xl px-5 text-center"><p className="text-sm font-extrabold uppercase tracking-[0.16em] text-[#a65d32]">Como funciona de verdade</p><h1 className="mt-4 text-4xl font-extrabold tracking-[-0.06em] text-[#2c1a12] sm:text-6xl">Você organiza a rotina. O Hubly mostra o que merece sua atenção.</h1><p className="mx-auto mt-6 max-w-3xl text-base leading-relaxed text-[#735f52] sm:text-lg">Não é só uma agenda. É um lugar para atender melhor, enxergar o financeiro, manter clientes por perto e tomar decisões com mais clareza.</p></div></section><section className="bg-[#f8f1e9] py-16 sm:py-20"><div className="mx-auto max-w-5xl px-5"><div className="grid overflow-hidden rounded-3xl border border-[#eaded2] bg-white md:grid-cols-2">{layers.map((layer, index) => <article key={layer.number} className={`p-7 sm:p-8 ${index > 0 ? "border-t border-[#eaded2]" : ""} ${index % 2 === 1 ? "md:border-l" : ""} ${index > 1 ? "md:border-t" : ""}`}><span className="text-sm font-black text-[#b17b50]">{layer.number}</span><h2 className="mt-5 text-2xl font-extrabold tracking-[-0.04em] text-[#352217]">{layer.title}</h2><p className="mt-3 text-sm leading-relaxed text-[#796659]">{layer.text}</p></article>)}</div><div className="mx-auto mt-10 max-w-3xl text-center"><p className="text-base font-semibold leading-relaxed text-[#4d3325]">Você pode começar pelo que mais pesa hoje na sua rotina e ativar novos recursos conforme o negócio cresce. O Hubly acompanha esse ritmo sem obrigar você a usar tudo de uma vez.</p><Button asChild className="mt-7 rounded-xl bg-[#3b2115] px-6 font-bold hover:bg-[#57321d]"><a href={CADASTRO_URL}>Começar agora <ArrowRight className="ml-2 h-4 w-4" /></a></Button></div></div></section></>;
}

function SupportPage({ page }: { page: Exclude<PublicPage, "inicio"> }) {
  if (page === "como-funciona") return <HowItWorksPage />;
  const intro = pageCopy[page];
  const details = supportDetails[page];
  return <><section className="relative overflow-hidden bg-[#fffaf4] py-20 sm:py-28"><div className="absolute right-[-6rem] top-[-8rem] h-80 w-80 rounded-full bg-[#f6d777]/30 blur-3xl" /><div className="relative mx-auto max-w-4xl px-5 text-center"><p className="text-sm font-extrabold uppercase tracking-[0.16em] text-[#a65d32]">{intro.eyebrow}</p><h1 className="mt-4 text-4xl font-extrabold tracking-[-0.06em] text-[#2c1a12] sm:text-6xl">{intro.title}</h1><p className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-[#735f52] sm:text-lg">{intro.description}</p><Button asChild className="mt-8 rounded-xl bg-[#3b2115] px-6 font-bold hover:bg-[#57321d]"><a href={CADASTRO_URL}>Começar agora <ArrowRight className="ml-2 h-4 w-4" /></a></Button></div></section><section className="bg-[#f8f1e9] py-16 sm:py-24"><div className="mx-auto max-w-6xl px-5"><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{details.map(({ label, title, text, icon: Icon }) => <article key={title} className="group rounded-3xl border border-[#eaded2] bg-white p-6 transition-transform hover:-translate-y-1"><div className="flex items-center justify-between"><span className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#a65d32]">{label}</span><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#f6ebdf] text-[#8a4d2b]"><Icon className="h-5 w-5" /></span></div><h2 className="mt-7 text-2xl font-extrabold tracking-[-0.04em] text-[#352217]">{title}</h2><p className="mt-3 text-sm leading-relaxed text-[#796659]">{text}</p></article>)}</div></div></section><section className="bg-[#fffdf9] py-16"><div className="mx-auto flex max-w-5xl flex-col items-center gap-6 px-5 text-center"><p className="max-w-2xl text-xl font-bold tracking-[-0.03em] text-[#3b2115]">O Hubly foi pensado para deixar a gestão mais simples, não para colocar mais uma ferramenta complicada na sua rotina.</p><Button asChild variant="outline" className="rounded-xl border-[#d7c5b5] bg-white px-6 font-bold text-[#4d3325] hover:bg-[#f8f1e9]"><a href="/">Voltar para o início <ArrowRight className="ml-2 h-4 w-4" /></a></Button></div></section></>;
}

function TopicPage({ topic }: { topic: PublicTopic }) {
  const content = publicTopics[topic];
  const Icon = content.icon;
  return <><section className="relative overflow-hidden bg-[#fffaf4] py-20 sm:py-28"><div className="absolute right-[-6rem] top-[-8rem] h-80 w-80 rounded-full bg-[#f6d777]/30 blur-3xl" /><div className="relative mx-auto max-w-4xl px-5 text-center"><p className="text-sm font-extrabold uppercase tracking-[0.16em] text-[#a65d32]">{content.eyebrow}</p><span className="mx-auto mt-7 grid h-14 w-14 place-items-center rounded-2xl bg-[#f4e5d5] text-[#8a4d2b]"><Icon className="h-6 w-6" /></span><h1 className="mt-5 text-4xl font-extrabold tracking-[-0.06em] text-[#2c1a12] sm:text-6xl">{content.title}</h1><p className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-[#735f52] sm:text-lg">{content.description}</p></div></section><section className="bg-[#f8f1e9] py-16 sm:py-24"><div className="mx-auto grid max-w-5xl gap-10 px-5 lg:grid-cols-[1.1fr_.9fr] lg:items-start"><div className="rounded-3xl border border-[#eaded2] bg-white p-7 sm:p-9"><p className="text-sm font-extrabold uppercase tracking-[0.14em] text-[#a65d32]">Na prática</p><ul className="mt-6 space-y-4 text-base leading-relaxed text-[#604b3d]">{content.benefits.map((benefit) => <li key={benefit} className="flex gap-3"><Check className="mt-1 h-4 w-4 shrink-0 text-[#5f9a59]" />{benefit}</li>)}</ul></div><aside className="rounded-3xl bg-[#3b2115] p-7 text-white sm:p-9"><p className="text-sm font-bold text-[#f7d187]">O resultado</p><p className="mt-5 text-2xl font-extrabold leading-tight">{content.outcome}</p><Button asChild className="mt-8 rounded-xl bg-[#f5d77a] px-6 font-bold text-[#432815] hover:bg-[#ffe493]"><a href={CADASTRO_URL}>Começar agora <ArrowRight className="ml-2 h-4 w-4" /></a></Button></aside></div></section><section className="bg-[#fffdf9] py-14"><div className="mx-auto flex max-w-4xl flex-col items-center gap-5 px-5 text-center"><p className="text-lg font-bold text-[#3b2115]">Conheça as outras partes da gestão que o Hubly deixa mais simples.</p><a href="/recursos" className="inline-flex items-center gap-1 text-sm font-bold text-[#8a4d2b] hover:underline">Ver todos os recursos <ArrowRight className="h-4 w-4" /></a></div></section></>;
}
