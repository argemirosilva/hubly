 ...
 * Como os Price IDs são criados dinamicamente, usamos o metadata do produto.
 * Fallback: retorna FREE se não reconhecer.
 */
function priceIdToPlanType(priceId: string): "FREE" | "SOLO" | "PLUS" | "PRO" {
  // Os Price IDs são armazenados como variáveis de ambiente após criação
  if (priceId === process.env.STRIPE_PRICE_SOLO_MENSAL) return "SOLO";
  if (priceId === process.env.STRIPE_PRICE_SOLO_ANUAL) return "SOLO";
  if (priceId === process.env.STRIPE_PRICE_PLUS_MENSAL) return "PLUS";
  if (priceId === process.env.STRIPE_PRICE_PLUS_ANUAL) return "PLUS";
  if (priceId === process.env.STRIPE_PRICE_PRO_MENSAL) return "PRO";
  if (priceId === process.env.STRIPE_PRICE_PRO_ANUAL) return "PRO";
  // Fallback: tenta inferir pelo metadata via API (já tratado no checkout.session.completed)
  return "SOLO";
}
