...
function HowItWorksPage() {
  return <><section className="relative overflow-hidden bg-[#fffaf4] py-20 sm:py-28"><div className="absolute right-[-6rem] top-[-8rem] h-80 w-80 rounded-full bg-[#f6d777]/30 blur-3xl" /><div className="relative mx-auto max-w-3xl px-5 text-center"><p className="text-sm font-extrabold uppercase tracking-[0.16em] text-[#a65d32]">Simples desde o primeiro dia</p><h1 className="mt-4 text-4xl font-extrabold tracking-[-0.06em] text-[#2c1a12] sm:text-6xl">Você organiza. O Hubly acompanha.</h1><p className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-[#735f52] sm:text-lg">Cadastre o básico, use a agenda no dia a dia e acompanhe clientes, pagamentos e mensagens no mesmo lugar. É só isso para começar.</p></div></section><section className="bg-[#f8f1e9] py-14 sm:py-18"><div className="mx-auto max-w-4xl px-5"><div className="grid overflow-hidden rounded-3xl border border-[#eaded2] bg-white md:grid-cols-3">{[{ number: "01", title: "Configure", text: "Serviços, horários e profissionais do seu jeito." }, { number: "02", title: "Atenda", text: "Organize a agenda e acompanhe cada cliente." }, { number: "03", title: "Acompanhe", text: "Veja pagamentos, confirmações e o que precisa de atenção." }].map((step, index) => <div key={step.number} className={`p-7 sm:p-8 ${index > 0 ? "border-t border-[#eaded2] md:border-l md:border-t-0" : ""}`}><span className="text-sm font-black text-[#b17b50]">{step.number}</span><h2 className="mt-5 text-2xl font-extrabold tracking-[-0.04em] text-[#352217]">{step.title}</h2><p className="mt-3 text-sm leading-relaxed text-[#796659]">{step.text}</p></div>)}</div><div className="mt-10 text-center"><p className="mx-auto max-w-2xl text-base font-semibold leading-relaxed text-[#4d3325]">Você não precisa usar tudo de uma vez. Comece pela rotina que já existe e adicione recursos quando fizer sentido.</p><Button asChild className="mt-7 rounded-xl bg-[#3b2115] px-6 font-bold hover:bg-[#57321d]"><a href="/admin">Começar agora <ArrowRight className="ml-2 h-4 w-4" /></a></Button></div></div></section></>;
}
